...
def generate_launch_description():
    ...
    # NEW: Camera info topic argument
    camera_info_topic_arg = DeclareLaunchArgument(
        'camera_info_topic',
        default_value='/a200_1103/sensors/camera_0/color/camera_info',
        description='Camera info topic for intrinsics (needed for 3D bounding box)'
    )
    ...
    crack_detection_node = Node(
        ...
        parameters=[
            config_file,  # Load YAML file FIRST (base configuration)
            {
                ...
                'camera_info_topic': LaunchConfiguration('camera_info_topic'),  # NEW
                'input_size': 384,
                'window_size': 384,
                'subdivisions': 2,
